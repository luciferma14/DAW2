function analizar() {

    let frase1 = prompt("Frase 1:")
    let frase2 = prompt("Frase 2:")
    let textoAnalizar = "";

    if (frase1.length >= frase2.length) {
        textoAnalizar = frase1;
        alert("Analizaremos la primera frase: '" + frase1 + "'");
    } else {
        textoAnalizar = frase2;
        alert("Analizaremos la segunda frase: '" + frase2 + "'");
    }

    let caracteresUnicos = "";

    for (let char of textoAnalizar) {

        if (!caracteresUnicos.includes(char) && char.trim() !== "") {
            caracteresUnicos += char + ", ";
        }
    }

    alert("En el texto '" + textoAnalizar + "', no se repiten estas letras " + caracteresUnicos);
}